...
public class CrawlDBTestUtil {
    ...
    public static void createCrawlDb(Configuration conf, FileSystem fs, Path crawldb, List<URLCrawlDatum> init)
        ...
        MapFile.Writer writer =
                new MapFile.Writer(conf, fs, new Path(dir, "part-00000").toString(), Text.class,
                    CrawlDatum.class);
     ...
     * to override the default one and it is currently not possible to use
...
//        conf.addResource("crawl-tests.xml");
    ...
    public static class URLCrawlDatum {
        ...
        public String toString() {
            return String.format("url: %s, fetach interval:%s,fetch time:%s,score:%s,status:%s", url,
                datum.getFetchInterval(), datum.getFetchTime(), datum.getScore(), datum.getStatus());
        }
    }
     ...
     * @throws IOException
     */
    public static void generateSeedList(FileSystem fs, Path urlPath, List<String> urls, List<String> metadata)
            throws IOException {
