...
public class Document extends Fragment {
	...
	private void extract(Document document, ExtractedDoc mainDoc, List<ExtractedDoc> extractedDocs, Object root, EvaluationContext context) throws Exception {
		...
		try {
			document.extractScriptFields(root, context, mainDoc);
		} catch (Exception e) {
			LOGGER.warn("javascript parse failure!" + e);			
		}

		try {
			document.extractJsonAttrs(root, context, mainDoc,extractedDocs);
		} catch (Exception e) {
			LOGGER.warn("json parse failure!" + e);
		}
	...
	public String toString() {
		return "Document [url=" + url + ", contentType=" + contentType + ", id=" + id + ", inherits=" + inherits + ", engine=" + engine + "]";
	}
