...
import me.alad.nutch.html.extractor.core.ExtractUtil;
import me.alad.nutch.html.extractor.protocol.ProtocolException.ProtocolErrorCode;
...
public class DirectHttpProtocol implements Protocol {
	...
	public void setConf(Config conf) {
		...
		userAgent = getAgentString(conf.get("http.agent.name"), conf.get("http.agent.version"), conf.get("http.agent.description"), conf.get("http.agent.url"),
	...
	protected HttpURLConnection getConnection(URL url, Map<String, Object> parameters) throws IOException {
		...
		/*
		 * Long lastModified = (Long) parameters.get(PARAM_LAST_MODIFIED); if
		 * (lastModified != null) {
		 * connection.setRequestProperty("If-Modified-Since",
		 * HttpDateFormat.toString(lastModified)); }
		 * 
		 * if (url.getUserInfo() != null) { String basicAuth = "Basic " + new
		 * String(new Base64().encode(url.getUserInfo().getBytes()));
	...
	protected InputStream getInputStream(HttpURLConnection connection, Map<String, Object> parameters) throws ProtocolException, IOException {
		connection.connect();
