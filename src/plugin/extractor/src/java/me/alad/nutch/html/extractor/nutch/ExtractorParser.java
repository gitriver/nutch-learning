...
public class ExtractorParser implements Parser {
	...
	public ParseResult getParse(Content content) {
	    LOGGER.info("testparse"+content.getUrl() + "------xxxxxxxxxxxxxxxxxxxxxxxx--------" + content.getContentType());
		ParseResult parseResult = new ParseResult(content.getUrl());
		try {
			List<ExtractedDoc> docs = ExtractEngine.getInstance().extract(
					new me.alad.nutch.html.extractor.core.Content(new URL(content.getUrl()), new ByteArrayInputStream(content.getContent()),
	...
	private ParseData getParseData(Content content, ExtractedDoc doc) throws MalformedURLException {
		...
		return new ParseData(ParseStatus.STATUS_SUCCESS, doc.getTitle(), NutchUtils.getOutlinksAsArray(doc.getOutlinks()), content.getMetadata(), parseMeta);
	}
