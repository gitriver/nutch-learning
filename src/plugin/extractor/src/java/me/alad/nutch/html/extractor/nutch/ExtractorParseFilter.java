...
public class ExtractorParseFilter implements HtmlParseFilter {
	...
	@Override
	public ParseResult filter(Content content, ParseResult parseResult, HTMLMetaTags metaTags, DocumentFragment documentFragment) {
		...
		try {
			...
			List<ExtractedDoc> extractedDocs = ExtractEngine.getInstance().extract(
					...
					new me.alad.nutch.html.extractor.core.Content(new URL(content.getUrl()), new ByteArrayInputStream(content.getContent()), encoding, content
							.getContentType()));
	...
	private void addExtractedDocToParseResult(Content content, ParseResult parseResult, ExtractedDoc doc) throws MalformedURLException {
		Parse parse = parseResult.get(doc.getUrl());
		Metadata parseMetadata = null;
		if (parse == null) {
			parseMetadata = new Metadata();
			parseResult.put(doc.getUrl(), new ParseText(doc.getText()),
					new ParseData(ParseStatus.STATUS_SUCCESS, doc.getTitle(), NutchUtils.getOutlinksAsArray(doc.getOutlinks()), content.getMetadata(),
							parseMetadata));
		} else
			parseMetadata = parse.getData().getParseMeta();
		// Indicates that this document is matched with one of urls defined in
		// the config.
		// This will be used in ParseMetadataIndexingFilter to decide whether
		// exclude document or not
		...
		if (doc.isSubDoc()) {
			content.getMetadata().set(MyConstant.NUTCH_SUB_DOC, "true");
			content.getMetadata().set(Nutch.FETCH_TIME_KEY, "" + System.currentTimeMillis());
		}
	}
